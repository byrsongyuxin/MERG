...
def load_checkpoint(model,path):
    ...
    for k,v in state_dict.items():
        if k in model_dict.keys() and v.shape == model_dict[k].shape:
            continue
        elif k not in model_dict.keys():
            print('model dont have: ', k)
        else:
            print('state_dict: ', k, v.shape, ' model_dict shape: ', model_dict[k].shape)
    
    
    state_dict = {k:v for k,v in state_dict.items() if k in model_dict.keys() and v.shape == model_dict[k].shape} 
    print('=> model_dict len: ', len(model_dict))
    print('=> update state_dict len: ', len(state_dict))
...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs, args):
    ...
    if args.pretrain is not None:
        model = load_checkpoint(model,path = args.pretrain)
        ...
        with tqdm(range(params['epochs'])) as t:
            for epoch in t:
                ...
                if epoch_test_f1 > best_test_f1:                 ######################################
                    ...
                    print('best_test_hits@50 update: ',best_test_f1)      ######################################
    ...
    print('load best model from: ', best_path) ######################################
...
def main():    
    ...
    ###########################################################
    # our
    parser.add_argument('--edge_init', default=None, type=str, help="the way of edge initial")
    parser.add_argument('--pretrain', default=None, type=str, help="the way of edge initial")
    ###########################################################
